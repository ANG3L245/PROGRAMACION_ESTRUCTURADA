...
def menuFinanzas(conexionBD, usuario_activo):
    ...
    while opc != "8":
        try:
            ...
            print(f"\t{COLOR_BORDE}║{RESET}  {COLOR_TEXTO}5.- Ver grafica (Ingresos/Gastos){RESET}                    {COLOR_BORDE}║{RESET}")
            print(f"\t{COLOR_BORDE}║{RESET}  {COLOR_TEXTO}6.- Guardar grafica como imagen{RESET}                      {COLOR_BORDE}║{RESET}")
...
def mostrarGrafica(conexionBD, usuario_activo):
    try:
        print(f"\n\t\t{COLOR_TITULO}::::: VER GRAFICA :::::{RESET}\n")
        print(f"\t{COLOR_TEXTO}1.- Ingresos{RESET}")
        print(f"\t{COLOR_TEXTO}2.- Gastos{RESET}")
        
        opc_tipo = input(f"\n\t{COLOR_TEXTO}Elige el tipo de movimiento (1/2):{RESET} ").strip()
        while opc_tipo not in ("1", "2"):
            print(f"\n\t{COLOR_ALERTA}...Opcion invalida, elige 1 o 2...{RESET}")
            opc_tipo = input(f"\n\t{COLOR_TEXTO}Elige el tipo de movimiento (1/2):{RESET} ").strip()
            
        tipo = "ingreso" if opc_tipo == "1" else "gasto"
        datos = crud_finanzas.obtenerMovimientosIndividuales(usuario_activo, tipo, conexionBD)
        
        if len(datos) > 0:
            etiquetas = [f"{d[0]} ({d[2]})" for d in datos]
            montos = [float(d[1]) for d in datos]
            graficas.verGraficaHistorial(etiquetas, montos, tipo)
        else:
            print(f"\n\t{COLOR_ALERTA}...No hay historial de {tipo}s registrado!...{RESET}")
            funciones.esperarTecla()
...
def exportarGrafica(conexionBD, usuario_activo):
    try:
        print(f"\n\t\t{COLOR_TITULO}::::: EXPORTAR GRAFICA :::::{RESET}\n")
        print(f"\t{COLOR_TEXTO}1.- Ingresos{RESET}")
        print(f"\t{COLOR_TEXTO}2.- Gastos{RESET}")
        
        opc_tipo = input(f"\n\t{COLOR_TEXTO}Elige el tipo a exportar (1/2):{RESET} ").strip()
        while opc_tipo not in ("1", "2"):
            print(f"\n\t{COLOR_ALERTA}...Opcion invalida, elige 1 o 2...{RESET}")
            opc_tipo = input(f"\n\t{COLOR_TEXTO}Elige el tipo a exportar (1/2):{RESET} ").strip()
        ...
        if len(datos) > 0:
            etiquetas = [f"{d[0]} ({d[2]})" for d in datos]
            montos = [float(d[1]) for d in datos]
            graficas.guardarGraficaHistorial(etiquetas, montos, tipo)
            funciones.esperarTecla()
        else:
            print(f"\n\t{COLOR_ALERTA}...No hay historial de {tipo}s registrado!...{RESET}")
            funciones.esperarTecla()
...
def exportarMovimientos(conexionBD, usuario_activo):
    try:
        ...
        ruta_carpeta = "C:/Users/pokky/Desktop/proyecto_final1/archivos"
